...
tables for use with `get_FVS_ready()`.

`get_FVS_ready()` fetches FVS Stand and FVS Tree tables from a
...
- fia_cond_subset:
  ...
  Dataframe. A subset of an FIA COND table; will be used in an inner
  join to filter the FVS_StandInit and FVS_TreeInit tables.

- type:

  Character string, one of "PLOT" or "COND" (default "PLOT"). Specifies
  whether FVS\_\<Stand/Tree\>Init_PLOT or FVS\_\<Stand/Tree\>Init_COND
  should be returned.
...
- to_db:
  ...
  Boolean. If TRUE (default), will write the filtered dataframes to an
  FVS-ready database that can be passed to
...
- output_database:
  ...
  Character string. External location for filtered data tables. Default
  is 'fvs_ready.db'
...
`get_FVS_ready()`: List of 2. \$FVS_StandInit is a dataframe of the
...
#> # ℹ 23 more variables: REGION <dbl>, FOREST <dbl>, PV_CODE <chr>,
...
#> #   SITE_SPECIES <chr>, SITE_INDEX <dbl>, PID <chr>, CN <chr>
#> 
#> $FVS_TreeInit
#> # A tibble: 170 × 29
...
#> # ℹ 160 more rows
#> # ℹ 21 more variables: SPECIES <dbl>, DIAMETER <dbl>, DG <dbl>, HT <dbl>,
...
#> #   BH_YEARS <dbl>, PID <chr>, TUID <chr>, CN <chr>
#> 
