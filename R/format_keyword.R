...
format_keyword <- function(keyword, ..., dtypes = NULL, round_to = 2){
  ...
  if(!is.null(dtypes)){
    ...
    fmt <- paste0('%10-s', paste0('%10', dtypes, collapse = ''))
  }else if(n_fields == 0){
    return(sprintf('%10-s', toupper(gsub(' ', '', keyword))))
  }else{
    fmt <- paste0('%10-s', paste0(rep('%10s', n_fields), collapse = ''))
  }
  return(do.call(sprintf, c(fmt, append(fields, values = toupper(gsub(' ', '', keyword)), after = 0))))
}
